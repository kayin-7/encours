...
char	*get_next_line(int fd)
{
	...
	if (lect == 0)
	{
		...
		if (r == 0) 
			return (0);
		lect = malloc(sizeof (char) * (ft_strlen(r) + 1));
		lect = ft_strcat(lect, r);
		lect[ft_strlen(lect)] = 0;
		free(r);
	}
	if (lect[0] == '\n')
	{
...
*/			return (lect);
		lect = ft_strjoin(lect, r);
	}
	s = malloc(sizeof (char) * (ft_strchr(lect, '\n') + 1));
	s = ft_substr(lect, 0, ft_strchr(lect, '\n'));
	lect = ft_substr(lect, ft_strchr(lect, '\n'), ft_strlen(lect);
	return(free(r),s);
}
